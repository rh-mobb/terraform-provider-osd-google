...
| `references/vp-terraform-rosa/` | ROSA HCP Terraform modules and RHCS provider reference | Machine pool variable structure (autoscaling_enabled, min/max_replicas), resource validators (RequiredTogether, Conflicting), and Terraform module patterns for mapping flat variables to nested provider attributes |
